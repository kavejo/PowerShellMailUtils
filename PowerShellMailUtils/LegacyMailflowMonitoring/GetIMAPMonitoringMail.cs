...
namespace SyntheticTransactionsForExchange.LegacyMailflowMonitoring
{
    ...
    public class GetIMAPMonitoringMail : Cmdlet
    {
        /// <summary>
        ...
        /// </summary>
        [Parameter(Mandatory = true, HelpMessage = @"Password to access the Mailbox")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                using (ImapClient client = new ImapClient(new ProtocolLogger(logStream)))
                {
                    ...
                    {
                        for (int currentMessage = 0; currentMessage < inbox.Count && messageReceived == false; currentMessage++)
                        {
                            Stopwatch operationTimer = new Stopwatch();
                            operationTimer.Start();
                            MimeMessage message = inbox.GetMessage(currentMessage);
                            ...
                            if (message.Subject.Contains(SubjectGuid.ToString()))
                            {
                                ...
                                client.Inbox.AddFlags(currentMessage, MessageFlags.Deleted, true);
            ...
            monitoringData.SetReceivingInformation(SubjectGuid, receivedDateTime, Protocol.IMAP);
